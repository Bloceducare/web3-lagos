...
const ADMINS = [
  { user: 'admin',    pass: 'w3lagos2026', name: 'Admin' },
  { user: 'reviewer', pass: 'w3lreview',   name: 'Reviewer' },
  { user: 'team',     pass: 'w3lteam26',   name: 'Team' },
]
...
type App = {
  ...
  submitted: string; reviewedAt?: string; reviewedBy?: string;
}
...
export default function AdminPage() {
  ...
  useEffect(() => {
    const n = sessionStorage.getItem('w3l_admin')
    if (n) { setLoggedIn(true); setAdminName(n); loadApps() }
  }, [])

  const loadApps = () => {
    const stored = JSON.parse(localStorage.getItem('w3l_applications') || '[]')
    if (!stored.length) {
      // Demo data
      const demos: App[] = [
        { ref: 'W3L-DEMO1', firstname: 'Chioma', lastname: 'Okafor', email: 'chioma@example.com', phone: '+234 801', country: 'Nigeria', city: 'Lagos', org: 'Binance', role: 'DeFi Analyst', twitter: '@chioma', track: 'defi', attend: 'physical', visa: false, notes: '', status: 'pending', submitted: new Date(Date.now()-86400000*2).toISOString() },
        { ref: 'W3L-DEMO2', firstname: 'Kwame', lastname: 'Asante', email: 'kwame@example.com', phone: '+233 244', country: 'Ghana', city: 'Accra', org: 'Polygon', role: 'DevRel', twitter: '@kwame', track: 'dev', attend: 'physical', visa: true, notes: 'Need visa', status: 'pending', submitted: new Date(Date.now()-86400000).toISOString() },
        { ref: 'W3L-DEMO3', firstname: 'Fatima', lastname: 'Ibrahim', email: 'fatima@example.com', phone: '+234 703', country: 'Nigeria', city: 'Abuja', org: '', role: 'Student', twitter: '', track: 'edu', attend: 'virtual', visa: false, notes: '', status: 'approved', submitted: new Date(Date.now()-86400000*3).toISOString(), reviewedAt: new Date().toISOString(), reviewedBy: 'Admin' },
        { ref: 'W3L-DEMO4', firstname: 'Emeka', lastname: 'Nwosu', email: 'emeka@example.com', phone: '+234 812', country: 'Nigeria', city: 'PH', org: 'CryptoNG', role: 'Founder', twitter: '@emeka', track: 'vc', attend: 'physical', visa: false, notes: '', status: 'rejected', submitted: new Date(Date.now()-86400000*4).toISOString(), reviewedAt: new Date().toISOString(), reviewedBy: 'Admin' },
      ]
      localStorage.setItem('w3l_applications', JSON.stringify(demos))
      setApps(demos)
    } else {
      setApps(stored)
    }
  ...
  const login = () => {
    ...
    if (match) {
      sessionStorage.setItem('w3l_admin', match.name)
      setLoggedIn(true); setAdminName(match.name); loadApps(); setLoginErr(false)
    } else {
      setLoginErr(true)
    }
  }
  ...
  const logout = () => {
    sessionStorage.removeItem('w3l_admin')
    setLoggedIn(false)
  }

  const changeStatus = (ref: string, status: 'approved' | 'rejected') => {
    const updated = apps.map(a => a.ref === ref
      ? { ...a, status, reviewedAt: new Date().toISOString(), reviewedBy: adminName }
      : a
    )
    setApps(updated)
    localStorage.setItem('w3l_applications', JSON.stringify(updated))
    if (selected?.ref === ref) setSelected(updated.find(a => a.ref === ref) || null)
  ...
  const exportCSV = () => {
    ...
    const rows = apps.map(a => [a.ref,a.firstname,a.lastname,a.email,a.phone,a.country,a.city,a.org||'',a.role,a.twitter||'',TRACK_LABELS[a.track]||a.track||'',ATTEND_LABELS[a.attend]||'',a.visa?'Yes':'No',a.status,a.submitted].map(v=>`"${String(v).replace(/"/g,'""')}"`).join(','))
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--black)' }}>
      <div style={{ background: 'var(--black2)', border: '1px solid var(--border2)', borderRadius: 16, padding: 40, width: 360, textAlign: 'center' }}>
        ...
        <div style={{ marginBottom: 14, textAlign: 'left' }}>
          ...
          <input style={input} value={user} onChange={e => setUser(e.target.value)} onKeyDown={e => e.key === 'Enter' && login()} placeholder="admin" />
        </div>
        <div style={{ marginBottom: 14, textAlign: 'left' }}>
          ...
          <input type="password" style={input} value={pass} onChange={e => setPass(e.target.value)} onKeyDown={e => e.key === 'Enter' && login()} placeholder="••••••••" />
        </div>
        <button onClick={login} style={{ width: '100%', background: 'var(--blue)', color: '#fff', fontWeight: 700, fontSize: 14, padding: 13, borderRadius: 8, border: 'none', marginTop: 6 }}>Sign In →</button>
        {loginErr && <p style={{ fontSize: 12, color: 'var(--red)', marginTop: 10 }}>Invalid credentials. Try admin / w3lagos2026</p>}
        <p style={{ fontSize: 11, color: 'var(--mid)', marginTop: 16 }}>Default: admin / w3lagos2026</p>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ background: 'var(--black)', minHeight: '100vh' }}>
      ...
      <div style={{ padding: '40px 5%' }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--mid)', marginBottom: 32 }}>Review, approve or reject registrations for Web3Lagos Conference 5.0</p>
        ...
        <div style={{ background: 'var(--black2)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
          ...
          {!filtered.length && (
            <div style={{ textAlign: 'center', padding: '64px 20px', fontSize: 14, color: 'var(--mid)' }}>No applications found.</div>
          )}
          {filtered.map(a => (
            <div key={a.ref} onClick={() => setSelected(a)} style={{ display: 'grid', gridTemplateColumns: '2fr 2fr 1.2fr 1.2fr 1fr 1.2fr', padding: '16px 20px', borderBottom: '1px solid var(--border)', cursor: 'pointer', transition: 'background 0.15s', alignItems: 'center' }}>
              <div>
              ...
              <div style={{ fontSize: 13, color: 'var(--mid)' }}>{a.city}, {a.country}</div>
              ...
              <div style={{ display: 'flex', gap: 8 }} onClick={e => e.stopPropagation()}>
                {a.status !== 'approved' && <button onClick={() => changeStatus(a.ref, 'approved')} style={{ padding: '5px 12px', borderRadius: 6, fontSize: 11, fontWeight: 700, border: 'none', background: 'rgba(0,194,160,.15)', color: '#00D4B0' }}>✓</button>}
                {a.status !== 'rejected' && <button onClick={() => changeStatus(a.ref, 'rejected')} style={{ padding: '5px 12px', borderRadius: 6, fontSize: 11, fontWeight: 700, border: 'none', background: 'rgba(229,57,53,.1)', color: '#E53935' }}>✗</button>}
          ...
          <div style={{ background: 'var(--black2)', border: '1px solid var(--border2)', borderRadius: 16, width: '100%', maxWidth: 560, maxHeight: '85vh', overflowY: 'auto' }}>
            ...
            <div style={{ padding: '24px 28px' }}>
              {[
                ...
                ['Submitted', new Date(selected.submitted).toLocaleString()],
                ...(selected.reviewedAt ? [['Reviewed', `${new Date(selected.reviewedAt).toLocaleString()} by ${selected.reviewedBy}`]] : []),
              ].map(([k, v]: any) => (
            ...
            <div style={{ padding: '20px 28px', borderTop: '1px solid var(--border)', display: 'flex', gap: 10 }}>
              {selected.status !== 'approved' && <button onClick={() => changeStatus(selected.ref, 'approved')} style={{ flex: 1, padding: 12, borderRadius: 8, fontSize: 13, fontWeight: 700, border: 'none', background: 'rgba(0,194,160,.2)', color: '#00D4B0' }}>✓ Approve</button>}
              {selected.status !== 'rejected' && <button onClick={() => changeStatus(selected.ref, 'rejected')} style={{ flex: 1, padding: 12, borderRadius: 8, fontSize: 13, fontWeight: 700, border: 'none', background: 'rgba(229,57,53,.15)', color: '#E53935' }}>✗ Reject</button>}
