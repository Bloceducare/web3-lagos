...
import usersDb from  "@servers/models/volunteers"
import { sendVolunteerEmail } from "@servers/mailer";
       ...
       await Promise.all(
        newArr.map(async (user) => {
...

         {
            ...
            sendVolunteerEmail
          const sendEmail = await  sendVolunteerEmail(
            user.email,
           
         
          );
          await  usersDb.updateOne({email:user.email}, {
            $set: {sentGrpLink: true}
        })
